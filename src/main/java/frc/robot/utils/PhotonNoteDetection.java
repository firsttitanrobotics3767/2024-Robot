...
import static frc.robot.utils.FieldConstants.notePositions;
...
public class PhotonNoteDetection extends SubsystemBase {
      ...
      new Pose3d[] {
        ...
        new Pose3d(
            ...
            new Rotation3d(
                ...
                Units.degreesToRadians(17.6),
                Units.degreesToRadians(0.0))),
      };

  // IF THE STREAM RESOLUTION IS CHANGED, CHANGE THESE AS WELL
  private final double cameraResolutionX = 640.0;
  private final double cameraResolutionY = 360.0;

  // IF THE LENSES ARE CHANGED, CHECK THESE
  private final double[] horizontalFOV =
      new double[] {Units.degreesToRadians(70)};
  private final double[] verticalFOV =
      new double[] {Units.degreesToRadians(102)};
  ...
  public void periodic() {
    ...
    for (int i = 0; i < notePoses.length; i++) {
      ...
      SmartDashboard.putBoolean("vision/Not Null Ring Pose-" + i, notePoses[i] != null);
    }
    for (int i = 0; i < allNotePoses.size(); i++) {
      SmartDashboard.putString("vision/All Detections-" + i, (new Pose2d(allNotePoses.get(i), new Rotation2d())).toString());
    }
    SmartDashboard.putNumber("vision/All Notes Length", allNotePoses.size());
  }
